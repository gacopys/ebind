...
// pausing, meta transitions to paused.
func TestScheduler_Pause_TransitionsToPaused(t *testing.T) {
	...
	// Complete step a — auto-transition should fire (last in-flight).
	emulateHook(t, wf, dag.ID(), "a", []byte(`2`), nil)

	// Poll meta until paused (or timeout).
	deadline := time.Now().Add(2 * time.Second)
	for time.Now().Before(deadline) {
		m, _, _ := wf.Store.GetMeta(ctx, dag.ID())
	...
	m, _, _ := wf.Store.GetMeta(ctx, dag.ID())
	t.Fatalf("DAG never transitioned to paused; status = %s", m.Status)
}
